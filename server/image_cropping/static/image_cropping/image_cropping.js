var image_cropping = (function ($) {
    ...
    function init() {
      $('input.image-ratio').each(function() {
        ...
        // there can be several. Deal with it gracefully.
            $image_input = $('input[name=' + field + ']:first');
    ...
    function makeCropper($image, org_width, org_height) {
        if (lastCropperImg) {
            lastCropperImg.closest('.jcrop-image').remove();
        }

        lastCropperImg = $image;
        ...
        $image.Jcrop(options);
          ...
          if ($image_input.length && $image_input.data('thumbnail-url') !== undefined) {
              var $image = $('<img>', {
                ...
                makeCropper($image, org_width, org_height);
          }
          ...
          $image_input.change(function() {
            ...
            var $image = $('<img>', {
                ...
                makeCropper($image, this.naturalWidth, this.naturalHeight);
                  });
